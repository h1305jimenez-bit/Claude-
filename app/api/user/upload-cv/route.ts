...
name, phone, linkedin, education (degree + institution), target_role (most recent job title or role they are applying for), seniority (one of: Intern/Junior/Mid-level/Senior/Lead/Manager/Director/Executive), salary_expectation, work_authorization.`,
    ...
    return { prefs: JSON.parse(jsonMatch[0]) as ExtractedPrefs };
  } catch (e) {
    return { prefs: {} as ExtractedPrefs, error: (e as { message?: string }).message ?? String(e) };
  }
}
    ...
    const msg = await anthropic.messages.create({
      ...
      messages: [{
        ...
        content: [
          ...
          {
            type: "text",
            text: "Extract and return the full text content of this CV. Output plain text only — no JSON, no markdown, no commentary.",
          },
        ],
      }],
    });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    step = "extract";
    const [{ prefs, error: prefsError }, cvText] = await Promise.all([
      extractPrefs(buffer),
      extractCvText(buffer),
    ]);
